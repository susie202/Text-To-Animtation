...
def text_to_sequence(text, symbols, as_token=False):
    # cleaner_names = [x.strip() for x in cleaner_names.split(",")]
    return _text_to_sequence(text, as_token, symbols)


def _text_to_sequence(text, as_token, symbols):
    ...
    while len(text):
        ...
        if not m:
            sequence += _symbols_to_sequence(_clean_text(text), symbols)
            break
        sequence += _symbols_to_sequence(
            _clean_text(m.group(1)), symbols
        )
